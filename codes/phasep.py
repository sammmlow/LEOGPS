...
## RINEX observations as values, with PRNs as the keys to the observations,  ##
## and with the epochs as the keys to the PRNs.                              ##
##                                                                           ##
##                                                                           ##
## OUTPUT:                                                                   ##
##                                                                           ##
...
## rnxdata[epoch][PRN] has two new key-value pairs added.                    ##
...
def phsmrk(rnxdata, rnxstep, goodsats, inps):
    ...
    for PRN in goodsats:
        ...
        for epoch in rnxproc:
            ...
            # Now, we check if the current PRN is being observed, and flag it.
            if PRN in rnxproc[epoch]:
                
                # Check if this is the first observation.
                if prev_epoch in rnxproc:
                    if PRN not in rnxproc[prev_epoch]:
                        rnxproc[epoch][PRN]['flag'] = 'start'
                else:
                    rnxproc[epoch][PRN]['flag'] = 'start'
                
                # Check if this is the final observation.
                if next_epoch in rnxproc:
                    if PRN not in rnxproc[next_epoch]:
                        rnxproc[epoch][PRN]['flag'] = 'end'
                else:
                    rnxproc[epoch][PRN]['flag'] = 'end'
                ...
                if prev_epoch in rnxproc and next_epoch in rnxproc:
                    if PRN not in rnxproc[prev_epoch]:
                        if PRN not in rnxproc[next_epoch]:
                            rnxproc[epoch][PRN]['flag'] = 'solo'
                    if PRN in rnxproc[prev_epoch]:
                        if PRN in rnxproc[next_epoch]:
                            rnxproc[epoch][PRN]['flag'] = 'none'
                ...
                if freqnum == 1:
                    if 'P1' in rnxproc[epoch][PRN]:
                        L1p = rnxproc[epoch][PRN]['P1'] # P1 code observable
                    elif 'C1' in rnxproc[epoch][PRN]:
                        L1p = rnxproc[epoch][PRN]['C1'] # P1 code observable
                    else:
                        print('Problem found, no C1 or P1 observable in: ')
                        print(str(epoch) + ' for satellite PRN ' + str(PRN))
                        return None
                    L2p = rnxproc[epoch][PRN]['L1'] # L1 phase observable
                    L2p = L2p * WL1 # Multiply phase by wavelength
                
                # Perform the geometry-free linear combination as the L4.
                elif freqnum == 2:
                    L1p = rnxproc[epoch][PRN]['L1'] # L1 phase observable
                    L1p = L1p * WL1 # Multiply phase by wavelength
                    L2p = rnxproc[epoch][PRN]['L2'] # L2 phase observable
                ...
                rnxproc[epoch][PRN]['L4'] = L4 # Throw in L4 value
    ...
    for PRN in goodsats:
        ...
        # Then check through each epoch based on a PRN ID
        for epoch in rnxproc:
            ...
            if PRN in rnxproc[epoch]:
                ...
                # Read the flag of this PRN observable
                obs = rnxproc[epoch][PRN]['L4']
                flag = rnxproc[epoch][PRN]['flag']
                
                if prev_epoch in rnxproc:
                    if PRN in rnxproc[prev_epoch]:
                        preflag = rnxproc[prev_epoch][PRN]['flag']
                else:
                    preflag = 'none'
                ...
                # First, let's check if it is the last entry of this PRN...
                if flag == 'end':
                ...
                # Otherwise then it is just a normal entry...
                    ...
                    print('Epoch and PRN ID:')
                    print(str(epoch) + ' ' + str(PRN))
...
def ph1fil(rnxdata, rnxstep, goodsats, hatchlen):
    ...
    # First, parsing across time, for each PRN.
    ...
    for PRN in goodsats:
        ...
        for epoch in rnxdata:
            ...
            if PRN in rnxdata[epoch]:
                
                trigger = True # Allow for code-carrier Hatch filtering
                flag = rnxdata[epoch][PRN]['flag'] # Check the flag
                
                # We also want to check if it is P1 or C1 entry
                if 'P1' in rnxdata[epoch][PRN]:
                    obstype = 'P1'
                    Ltype = 'L1'
                elif 'C1' in rnxdata[epoch][PRN]:
                    obstype = 'C1'
                    Ltype = 'L1'
                else:
            ...
            else: # Ignore this entry if there is no PRN
                ...
                if flag == 'solo':
                    ...
                    if slipcheck == False:
                        ...
                        for n in range(0,k,-1): # Sliding window hatch filter
                            ...
                            R1 = rnxdata[epoch+((n)*rnxstep)][PRN][obstype]
                            R2 = rnxdata[epoch+((n-1)*rnxstep)][PRN][obstype]
                            P1 = rnxdata[epoch+((n)*rnxstep)][PRN][Ltype]
                            P2 = rnxdata[epoch+((n-1)*rnxstep)][PRN][Ltype]
                            kM = np.abs(k) + 1
                            
                            # Generate the new smoothed code value
                            RNew = hatch1(R1,R2,P1,P2,kM,Ltype)
                            rnxdata[epoch+((n)*rnxstep)][PRN][obstype] = RNew
                ...
                if flag == 'solo':
                    ...
                    if slipcheck == False:
                        ...
                        for n in range(0,k,-1): # Sliding window hatch filter
                            ...
                            # Get current and previous code-carrier values
                            RF = rnxdata[epoch+((n)*rnxstep)][PRN][obs1]
                            RI = rnxdata[epoch+((n-1)*rnxstep)][PRN][obs1]
                            
                            # Get L1 phase values now and before
                            PF1 = rnxdata[epoch+((n)*rnxstep)][PRN]['L1']
                            PI1 = rnxdata[epoch+((n-1)*rnxstep)][PRN]['L1']
                            
                            # Get L2 phase values now and before
                            PF2 = rnxdata[epoch+((n)*rnxstep)][PRN]['L2']
                            PI2 = rnxdata[epoch+((n-1)*rnxstep)][PRN]['L2']
                            ...
                            rnxdata[epoch+((n)*rnxstep)][PRN][obs1] = RNew
